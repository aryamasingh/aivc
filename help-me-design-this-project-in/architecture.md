...
This is a first-draft diligence prototype, not a final investment decision system. Live research quality depends on search results, private metrics such as ARR/NRR/gross margin are usually unavailable, and some extraction still uses deterministic heuristics. In production, I would strengthen entity resolution, source retrieval, structured validation, and evaluation coverage before using it in an IC process.
