...
def _infer_product_from_text(text: str) -> str:
    ...
    if _contains_any(lower, ["cybersecurity", "security operations", "security software", "compliance software", "grc"]):
        return "cybersecurity, compliance, and security workflow software"
...
def _competitor_framework(product: str, customers: list[str] | str) -> dict[str, Any]:
    ...
    if "collaborative product design" in product:
        return {
            ...
            "primary_buyer": customer_text if customer_text != "not specified" else "design, product, engineering, and enterprise software teams",
            "product_category": "collaborative product design software",
            "known_competitors": ["Adobe", "Canva", "Miro", "Sketch", "Framer", "Atlassian"],
            "competitor_groups": {
                "Direct Competitors": ["Sketch", "Framer"],
                "Adjacent Competitors": ["Canva", "Miro"],
                "Incumbents": ["Adobe", "Atlassian"],
            },
            "exclusions": ["ERP systems", "academic collaboration portals", "generic workflow automation vendors", "healthcare software"],
        }
    if "radiology report generation" in product:
        return {
...
def _note_anchor_profile(
    ...
    workflow = _competitor_framework(product, customers or ["not specified"])["primary_workflow"]
    traction_terms = ["customer", "customers", "adoption", "pilot", "pilots", "usage", "arr", "revenue", "growth"]
    team_terms = ["founder", "team", "executive", "sold a startup", "prior company", "operator"]
    market_terms = ["interesting because", "market", "category", "large", "urgent", "tailwind", "why now"]
    return {
        "target_company": company,
        "product_description": "Unknown" if product == "Not specified" else product,
        "target_customer": ", ".join(customers) if customers else "Unknown",
        "core_workflow": workflow if not _is_placeholder_entity(workflow) else "Unknown",
        "market_thesis": _first_matching_note_sentence(raw_notes, market_terms) or "Unknown",
        "team_signal": _first_matching_note_sentence(raw_notes, team_terms) or "Unknown",
        "traction_claim": _first_matching_note_sentence(raw_notes, traction_terms) or "Unknown",
        "key_diligence_questions": open_questions or _extract_note_questions(raw_notes),
        "sector_from_notes": sector,
        "risks_from_notes": risks,
    }


def _first_matching_note_sentence(raw_notes: str, terms: list[str]) -> str:
    for sentence in re.split(r"(?<=[.!?])\s+|\n+", raw_notes):
...
def _extract_note_questions(raw_notes: str) -> list[str]:
    questions = []
    capture = False
    for line in raw_notes.splitlines():
        ...
        if lower.startswith("need to understand") or lower.startswith("need to verify"):
            capture = True
            remainder = stripped.split(":", 1)[1].strip() if ":" in stripped else ""
            if remainder:
                questions.append(remainder)
            continue
        if capture and (line.lstrip().startswith("-") or stripped.endswith("?")):
            questions.append(stripped.rstrip("?"))
    return questions
...
def _company_understanding(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    understanding = {
        ...
        "note_anchor": profile.get("note_anchor", {}),
        "category": sector,
        "industry": sector,
...
def _diligence_topics_for_profile(profile: dict[str, Any]) -> list[str]:
    ...
    if sector == "Product Design Software":
        ...
        return [
            "enterprise penetration and seat expansion",
            "AI roadmap and impact on design workflows",
            "retention and product-love durability",
            "monetization across design, product, and engineering teams",
...
def extract_company_profile(state: DealMemoState) -> dict[str, Any]:
    ...
    note_anchor = _note_anchor_profile(raw, company, product, customers, sector, risks, open_questions)
...
def plan_research(state: DealMemoState) -> dict[str, Any]:
    ...
    if identity and not identity.get("is_resolved", False):
        ...
        company_for_search = profile.get("raw_company_name") or company
        note_terms = " ".join(
            str(item)
            for item in [
                product if product not in {"Not specified", "Unknown", "Clarification required"} else "",
                workflow if workflow not in {"Not specified", "Unknown", "Clarification required"} else "",
                buyer if buyer not in {"not specified", "Unknown", "Clarification required"} else "",
            ]
            if item
        )
        plan = [
            {
                "query": f"{company_for_search} official website {note_terms}".strip(),
                "evidence_type": "website",
                "rationale": "Identity is unresolved, so search for company-specific sources that can verify the correct entity.",
            },
            {
                "query": f"{company_for_search} company funding investors news".strip(),
                "evidence_type": "news",
                "rationale": "Find public signals that may disambiguate the company entity before making company-specific claims.",
            },
            {
                "query": f"{company_for_search} Crunchbase funding".strip(),
        ...
        return {
            ...
            "trace": _trace(
                ...
                {
                    "queries": plan,
                    "mode": "identity_discovery",
                    "reason": "Company identity or workflow understanding is weak; run limited public research for disambiguation only.",
                },
            ),
        }
    plan: list[ResearchQuery] = [
        ...
        if understanding_confidence < 0.8:
            ...
            note_terms = " ".join(
                ...
                for item in [
                    product if product not in {"Not specified", "Unknown", "Clarification required"} else "",
                    workflow if workflow not in {"Not specified", "Unknown", "Clarification required"} else "",
                    understanding.get("target_buyer", "") if understanding.get("target_buyer") not in {"not specified", "Unknown", "Clarification required"} else "",
                ]
                if item
            )
            plan = [
                {
                    "query": f"{company_for_search} official website {note_terms}".strip(),
                    "evidence_type": "website",
                    "rationale": "Continue identity discovery with company-specific sources before using public data for memo claims.",
                },
                {
                    "query": f"{company_for_search} product customers market {note_terms}".strip(),
                    "evidence_type": "market",
                    "rationale": "Look for market context that can clarify product and buyer, while preserving low confidence.",
                },
                {
                    "query": f"{company_for_search} competitors alternatives {note_terms}".strip(),
                    "evidence_type": "competitor",
                    "rationale": "Use only as tentative competitor context until identity and workflow are confirmed.",
                },
            ]
            return {
                "research_plan": plan,
                "trace": _trace(
                    ...
                    "plan_context_research",
                    {
                        "queries": plan,
                        "mode": "identity_discovery",
                        "reason": "Company identity or workflow understanding is weak; gather disambiguation evidence without making strong claims.",
                    },
                ),
            }
        plan: list[ResearchQuery] = [
...
def refine_company_profile_from_evidence(state: DealMemoState) -> dict[str, Any]:
    ...
    if evidence_product != "Not specified" and note_scope == "broad_platform" and not _is_placeholder_entity(note_product):
    ...
    elif evidence_product != "Not specified" and note_product_known:
        profile["public_product_use_cases"] = public_use_cases
        ...
        profile["public_product_use_cases"] = public_use_cases
        profile["product_evidence_conflicts"] = ["Public product evidence was weak or ambiguous; note-derived framing retained."]
        profile["product_source_ids"] = []
        profile["product_source_basis"] = "partner notes only; public evidence ambiguous"
    else:
        profile["public_product_use_cases"] = public_use_cases
        profile["product_evidence_conflicts"] = product_conflicts
        profile["product_source_ids"] = []
...
def _next_diligence_priorities(company: str, profile: dict[str, Any]) -> dict[str, list[dict[str, str]]]:
    ...
    return _ground_diligence_priorities(profile.get("raw_notes", ""), {
        ...
        "Product / integration": [
            "Review implementation timeline, required system integrations, data requirements, and services load.",
        ...
        "Regulatory / security": [
            f"Verify {terms['security']}.",
            "Confirm customer contracting constraints, security requirements, compliance obligations, and risk allocation.",
        ],
    })
    ...
    memo = {
        ...
        "opportunity_scorecard": [
            ...
            _score_or_unknown(
                ...
                [
                    _factor("Strategic buyer fit", 6 if news_sources else None, f"Strategic fit is directional from {sector.lower()} news/context.", news_sources),
